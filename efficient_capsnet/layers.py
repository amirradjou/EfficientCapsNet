...
class FeatureMap(tf.keras.layers.Layer):
    ...
    def call(self, input_images: tf.Tensor) -> tf.Tensor:
        feature_maps = self.conv1(input_images)
        feature_maps = self.norm1(feature_maps)
        feature_maps = self.conv2(feature_maps)
        feature_maps = self.norm2(feature_maps)
        feature_maps = self.conv3(feature_maps)
        feature_maps = self.norm3(feature_maps)
        feature_maps = self.conv4(feature_maps)
        return self.norm4(feature_maps)

    def compute_output_shape(self,
                             input_shape: tf.TensorShape) -> tf.TensorShape:
        output_shape = self.conv1.compute_output_shape(input_shape)
        output_shape = self.norm1.compute_output_shape(output_shape)
        output_shape = self.conv2.compute_output_shape(output_shape)
        output_shape = self.norm2.compute_output_shape(output_shape)
        output_shape = self.conv3.compute_output_shape(output_shape)
        output_shape = self.norm3.compute_output_shape(output_shape)
        output_shape = self.conv4.compute_output_shape(output_shape)
        return self.norm4.compute_output_shape(output_shape)
...
class PrimaryCap(tf.keras.layers.Layer):
    ...
    def call(self, inputs: tf.Tensor) -> tf.Tensor:
        dconv_outputs = self.dconv(inputs)
        reshaped_outputs = self.reshape(dconv_outputs)
        return self.squash(reshaped_outputs)
